...
\begin{itemize}
	...
	\item \textbf{\glock{Proponente}:} RedBabel;
...
\begin{itemize}
	\item \glock{EML-FE}: è il componente per il front-end del sito, sviluppato usando la libreria \verb|Next.js| con il linguaggio \verb|Typescript|.
	\item \glock{EML-BE}: è il componente per la parte di back-end della piattaforma, usa lo stesso linguaggio del modulo precedente basandosi sui servizi \verb|AWS Lambda|.
	\item \glock{EML-I}: è il modulo per l'integrazione dei servizi di terze parti, utili ad esempio per le operazioni di pagamento e per l'autenticazione degli utenti. Questo deve essere sviluppato tramite il framework \textit{Serverless} nello stesso linguaggio degli altri due.
	\item \glock{EML-MON}: modulo per il monitoraggio del sistema da parte dell'\glock{amministratore}. I proponenti suggeriscono di implementarlo con \verb|Amazon CloudWatch|.
\end{itemize}
