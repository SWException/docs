...
Applicazione open source per costruire \glock{diagrammi di Gantt}, usati largamente nelle attività di pianificazione e presenti nel \dext{PianoDiProgetto\_3.0.0}.
...
\subsubsection{Descrizione}
Per esporre i valori di soglia delle metriche e gli standard da applicare al progetto, è stato introdotto il documento \dext{PianoDiQualifica\_3.0.0}.
...
Il codice viene verificato con test automatici e con misurazioni quantificabili specificate all'interno del \dext{PianoDiQualifica\_3.0.0}. Il codice quindi deve obbedire a questi criteri:
\begin{itemize}
    \item deve essere corretto rispetto ai test definiti e alle metriche adottate per quantificare la sua qualità;
    \item deve derivare dai requisiti richiesti in \dext{AnalisiDeiRequisiti\_3.0.0}.
\end{itemize}
...
\begin{itemize}
    ...
    \item siano coerenti con gli accordi presi con il proponente, riportati all'interno del documento \\\dext{AnalisiDeiRequisiti\_3.0.0}.
\end{itemize}
...
\subsubsection{Test}
I test costituiscono l'attività fondamentale dell'analisi dinamica: il loro utilizzo permette di individuare bug e di dimostrare che l'applicativo sviluppato è conforme rispetto a tutti i requisiti concordati nell'\dext{AnalisiDeiRequisiti\_3.0.0}.\\
...
Dopo che tutte le unità sono state testate ed è stata testata la loro integrazione viene eseguito il test dell'intero sistema. In questa fase si verifica che tutte le componenti interagiscano nel modo atteso e che l'applicazione soddisfi ciò che è stato definito nell'\dext{AnalisiDeiRequisiti\_3.0.0}.
I test di sistema dell'applicazione EmporioLambda saranno implementati con \glock{Selenium} e sono stati definiti nel \dext{PianoDiQualifica\_3.0.0} durante l'attività di analisi dei requisiti.
